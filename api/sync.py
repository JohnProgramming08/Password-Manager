...
class Sync:
    ...
    def user_exists(self) -> bool:
        user_dir_path = f"{self.users_path}/{self.email_hash}"
    ...
    def overwrite_section(self) -> bool:
        try:
            file_path = f"{self.users_path}/{self.email_hash}/{self.file_name}"
    ...
    def get_all_sections(self) -> list:
        try:
            dir_path = f"{self.users_path}/{self.email_hash}"
    ...
    def get_section_data(self) -> dict:
        try:
            section_path = (
                f"{self.users_path}/{self.email_hash}/{self.file_name}"
            )
